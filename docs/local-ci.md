...
scoped to resources this run created. It is not `docker system prune` and must never become one. Two
concurrent runs, or two repositories using this pattern, cannot collide.
...
```json
{
  ...
  "result": "passed",
  "environment": { "runner": "docker", "image": "hfn-local-ci:node20", "network": "none", "pipeline": "ci/run-checks.sh" },
  "startedAt": "...",
  "completedAt": "...",
