<!--
SOURCE DOCUMENT — COMMITTED VERBATIM. DO NOT EDIT.

This is the design brief for this repository, reproduced exactly as it was given. Together with
`original-prompt.md` (the domain content specification) it is the pair of inputs from which every
other artifact in this repository is derived.

`original-prompt.md` says WHAT the standards must cover. This document says WHAT KIND OF SYSTEM the
repository must be. Where a derived artifact and a source document disagree, the source document
wins and the derived artifact is the defect.

Corrections, clarifications, and decisions taken against this brief live in `artifacts/adr/` and in
`docs/design/architecture.md` — never as edits here. See ADR 0004.
-->
