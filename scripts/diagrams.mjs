 ...
 *
 * ADR 0006 makes the `.mmd` canonical. A rendered `.svg` and an embedded fence are both derived,
 * and a derived copy that no longer matches its source is the drift the ADR exists to prevent —
 * silently, because a stale diagram looks exactly like a current one.
 *
 * The design constraint worth naming: THIS CHECK REQUIRES NO MERMAID TOOLCHAIN. It compares text,
 * not pictures. That is what lets a zero-dependency repository enforce the rule at all; a check
 * that needed a headless browser would not run in this CI, and the rule would go unenforced.
 *
...
/** Marker a renderer must embed so an SVG can be traced to the source it came from. */
...
export async function check(root) {
  ...
  for (const source of sources) {
    ...
    // pair a .mmd to a particular document. A comparable implementation matched a host document by
    // comparing first lines and only then checked the rest — so editing the first line
    // (`flowchart TB` to `flowchart LR`) matched no host, and the drift was reported as clean. Its
    // mutation test caught it. A check whose failure path can be stepped around by the very edit it
    // guards against is worse than no check, because it reports green.
    ...
    if (!allBlocks.includes(text)) {
      findings.push({
        ...
        remediation: "Re-embed the .mmd verbatim in its document. The .mmd is canonical (ADR 0006).",
      });
    }
    ...
    try {
      ...
      // No render committed. ADR 0006 permits this where the toolchain is unavailable, provided the
      // absence is declared — a documentation obligation, not something checkable here.
    }
    if (svgText !== null) {
  ...
  // 3. A hand-authored SVG with no .mmd beside it is what ADR 0006 abolished.
  for (const file of files.filter((f) => f.endsWith(".svg"))) {
    ...
    if (!sources.includes(source)) {
      findings.push({
        ...
        remediation:
          "Diagrams are authored in Mermaid and rendered (ADR 0006). If this is not a diagram, move it out of a diagram path.",
      });
    }
  }
