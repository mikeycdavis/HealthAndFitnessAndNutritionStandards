 ...
 * alongside a schema is two definitions, and the drift between them is silent.
 *
 ...
 * checked — a false green in its purest form, and in this repository the document in question is
 * the one carrying applicability declarations and waivers. If a future schema adds `oneOf`, this
 * module fails loudly until someone implements it.
 *
...
export function assertSchemaSupported(schema) {
  const walk = (node, path) => {
    ...
    for (const [key, child] of Object.entries(node)) {
      // Below `properties` and `$defs` the keys are names, not keywords.
