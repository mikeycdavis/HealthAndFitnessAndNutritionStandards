 ...
 * so a policy file has to be readable without one.
 *
 * Why *strict*: an under-validating parser is a false green. A permissive parser that silently
 * misreads `expires: 2026-12-31` as a Date, or quietly ignores an anchor, would report a policy as
 * valid while it means something other than what its author wrote — and in this repository a policy
 * is where applicability declarations, waivers, and attestations live. Every construct outside the
 * supported subset is therefore a hard error rather than a best guess.
 *
 ...
 * schema declares that `standardVersion` is a string matching a semver pattern, and a parser that
 * turned `1.0` into a number would defeat that check before it ran.
 *
 * This file is vendored: it was adopted once as source and is maintained here. Nothing in it is
 * specific to health, fitness, or nutrition.
 */
